...
public class ColliderGen : MonoBehaviour
{
	...
	public void Test()
	{
		...
		for (int x = 0; x < 64; x++)
		{
			for (int y = 0; y < 64; y++)
			{
				...
				if (map.grid[x,y,0] != null && map.grid[x,y,0].colider)
				{
					List<Vector2> newPolygons = new List<Vector2>();
					newPolygons.Add(new Vector2(0 + x, 0 + y));
					newPolygons.Add(new Vector2(0 + x, 1 + y));
					newPolygons.Add(new Vector2(1 + x, 1 + y));
					newPolygons.Add(new Vector2(1 + x, 0 + y));

					polygons.Add(newPolygons); 

				}
			}
		}

		//CreateLevelCollider(UniteCollisionPolygons(polygons));
	...
	public List<List<Vector2>> UniteCollisionPolygons(List<List<Vector2>> polygons)
	{
		...
		{
			Path newPoligons = new Path(polygons[i].Count);

			for (int j = 0; j < polygons[i].Count; j++)
			{
				newPoligons.Add(new IntPoint(Mathf.Floor(polygons[i][j].x * scalingFactor), Mathf.Floor(polygons[i][j].y * scalingFactor)));
			}
